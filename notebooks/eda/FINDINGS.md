...
## Notebook 01 — Energy mix evolution (01_energy_mix_history)
   ...
   ### Headline findings
      ...
      7. **Total generation declined by ~9.7 GW, almost exactly matching
         ...
         Renewable additions (+4 GW combined renewables (mostly solar)) only partially
...
## Notebook 02 — Renewable Seasonality (02_renewable_seasonality)
   ...
   Analysis of the seasonal and diurnal patterns of German wind and
   solar generation across 2019–2025, combining six renewable SMARD
   signals (wind onshore/offshore, solar, biomass, hydropower, pumped
   storage) with weather data from four asset-aligned regions
   (Brandenburg, Schleswig-Holstein, Bavaria, Baden-Württemberg).
...
## Notebook 03 — Consumption Patterns (03_consumption_patterns)
   ...
   ### Headline findings
      ...
      7. **Consumption response to temperature is near-symmetric and V-shaped.**
         ...
         18°C is +453 MW/°C. The cooling sensitivity is essentially identical in magnitude to
         heating sensitivity despite Germany's historically low AC penetration.
         This is partly a confounding artifact — hot temperatures correlate with
         summer afternoon hours where industrial and commercial activity peaks
         independently of temperature — and partly a real signal of growing
         cooling demand. Without controlling for hour-of-day and day-of-week,
         the headline elasticity numbers are descriptive, not causal. Total
         heating impact remains larger than cooling impact only because
         Germany's temperature distribution extends further below the
         comfort zone than above it. Total heating impact remains larger only
...
## Notebook 04 - Price Dynamics (04_price_dynamics)
   ...
   ### Headline findings
      ...
      1. **Three structurally distinct price regimes exist in the data.**
         ...
         mean €216, median €184, std €142, with 5% of hours exceeding €500
      ...
      7. **The 2022 crisis mechanism:** Gas became the marginal generator for
         ...
         The return to lower prices in 2023 was driven primarily by gas price
         normalization (post-invasion gas prices fell from peak ~€300/MWh to
         ~€40 by mid-2023), demand destruction (notebook 03: −5.3 GW peak to
         trough), and LNG import capacity ramp. The nuclear phase-out happened
         in the same year by coincidence but pushed in the opposite direction —
         removing 8 GW of zero-marginal-cost baseload raised structural price
         pressure that imports absorbed (notebook 01). The post-2023 era is
         therefore a regime of "moderate average prices with elevated
         volatility" rather than a return to pre-crisis stability.
...
## Cross-cutting themes
   ...
   The five thematic notebooks converge on three structural insights
   that no single notebook fully captures alone.

   ### Theme 1 — The morning supply gap

      Notebooks 02, 03, and 04 independently identify the same operational
      window as the German grid's hardest hours: 05:00–12:00 in winter.

      - **Consumption ramps fast (notebook 03):** From 43.7 GW at 03:00 to
         62.4 GW at 11:00 — a 2.6 GW/hour ramp for six consecutive hours,
         the steepest sustained demand growth of the day.
      - **Renewables are at their daily low (notebook 02):** Wind onshore
         troughs at 10–11h (10.7 GW vs evening peak 12.8 GW); solar is
         ramping but does not exceed 4 GW until 09:00 even in summer, and
         averages near zero through this window in winter.
      - **Prices peak here (notebook 04):** Winter diurnal median prices
         peak at 09–11h, not in the evening — the reverse of residential-grid
         expectations, driven by the consumption ramp meeting low renewable
         output.

      This is not a "renewable intermittency" finding — it is a specific
      structural mismatch between when German industry needs power and
      when German renewables can provide it. Operationally, this window
      is when import dependency is highest, neighbour spreads are most
      informative, and ML model accuracy matters most for Zephyrwerk.

   ### Theme 2 — The duck curve

      The same midday pattern shows up at four different scales across
      four notebooks:

      - **Generation (notebook 02):** Solar peaks at 12–13h, pushing total
         renewable output to its diurnal maximum.
      - **Residual load (notebook 03):** Residual load drops to its daily
         minimum at 12–14h as solar covers an increasing fraction of demand.
      - **Prices (notebook 04):** Day-ahead prices form a midday trough,
         reaching negative territory in summer.
      - **Cross-border flows (notebook 05):** DE/LU goes negative against
         several neighbours at 10–15h — exporting solar surplus.

      The duck curve has migrated from a California-specific concept to a
      core feature of the German market. Each notebook's local finding is
      the same physical phenomenon observed at a different layer of the
      energy system.

   ### Theme 3 — The post-nuclear regime as a structural break

   Four notebooks document a coherent regime change centered on
   April 2023:

   - **Notebook 01:** Germany flips from net exporter to net importer.
   - **Notebook 03:** Residual load mean drops 22% while its variability
      rises 56% — a structurally more volatile system.
   - **Notebook 04:** Price distribution shifts to a new bimodal shape;
      volatility doubles relative to pre-crisis.
   - **Notebook 05:** Neighbour spreads become more informative as
      Germany loses domestic baseload anchor.

   These are not four separate findings — they are four observations of
   the same regime change in different signals. The ML training-window
   restriction to ≥ 2023-04-16 is not arbitrary: the data before that
   date describes a system that no longer exists.